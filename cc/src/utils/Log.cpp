...
namespace cc::debug
{
	...
	void Logger::init()
	{
		...
		auto LogFileSink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>("cc.log", 1048576 * 5, 3);
		Sinks.emplace_back(LogFileSink); // Log file
		// create the loggers
		CoreLogger = std::make_shared<spdlog::logger>("CC", begin(Sinks), end(Sinks));
